...
function displayThread(threadData) {
    ...
    document.getElementById('questionDate').textContent = `Asked on ${formattedDate}`;

    // Display answers (tutor replies) with their follow-ups
        ...
        answers.forEach((answer, index) => {
            ...
            answerCard.innerHTML = `
                <div class="answer-header">
                    <span class="answer-number">Reply #${index + 1}</span>
            ...
            // Display follow-ups for this answer (if student viewing)
            if (!isTutor && answer.followUps && answer.followUps.length > 0) {
                const followUpContainer = document.createElement('div');
                followUpContainer.className = 'followup-container';
                followUpContainer.innerHTML = '<div class="followup-label">📝 Student Follow-ups:</div>';
                ...
                answer.followUps.forEach((followUp, fIndex) => {
                    ...
                    followUpCard.innerHTML = `
                        <p class="followup-text">${escapeHtml(followUp.followup_body)}</p>
                        <span class="followup-date">${fFormattedDate}</span>
                    `;
            ...
            // Add follow-up form for students under each answer
            if (!isTutor) {
    ...
    // Show tutor reply section if user is tutor, hide student follow-up section
    if (isTutor) {
        document.getElementById('tutorReplySection').classList.remove('hidden');
        document.getElementById('studentFollowUpSection').classList.add('hidden');
    } else {
        // Hide tutor section for students, show follow-up section
...
async function submitReply() {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/course/${courseId}/forum/threads/${threadId}/reply`, {
            ...
            body: JSON.stringify({ answerBody: replyText })
        });
...
async function submitFollowUp(answerId) {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/course/${courseId}/forum/answers/${answerId}/followup`, {
            ...
            body: JSON.stringify({ followupBody: followUpText })
        });
...
function goBackToForumList() {
    ...
    const userRole = localStorage.getItem('userRole') || 'Undergraduate';
